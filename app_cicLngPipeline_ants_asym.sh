...
if [ ${tp} -gt 1 ];then
    if [ ! -f ${path_nlin_av} ];then
        ...
        for iteration in {1..5};do
            ...
            ### Making the average symmetric ###
            flip_volume ${output_path}/${id}/template/${id}_lin_av.mnc ${output_path}/${id}/template/${id}_lin_av_f.mnc
            mincaverage ${output_path}/${id}/template/${id}_lin_av.mnc ${output_path}/${id}/template/${id}_lin_av_f.mnc ${output_path}/${id}/template/${id}_lin_av_sym.mnc
            mv ${output_path}/${id}/template/${id}_lin_av_sym.mnc ${output_path}/${id}/template/${id}_lin_av.mnc
        done
...
if [ ${tp} -gt 1 ];then 
    ...
    if [ ! -f ${path_nlin_av} ];then  
        ...
        for iteration in {1..4};do
            ...
            ### Making the average symmetric ###
            flip_volume ${output_path}/${id}/template/${id}_nlin_av.mnc ${output_path}/${id}/template/${id}_nlin_av_f.mnc
            mincaverage ${output_path}/${id}/template/${id}_nlin_av.mnc ${output_path}/${id}/template/${id}_nlin_av_f.mnc ${output_path}/${id}/template/${id}_nlin_av_sym.mnc
            mv ${output_path}/${id}/template/${id}_nlin_av_sym.mnc ${output_path}/${id}/template/${id}_nlin_av.mnc
        done
    fi
...
for timepoint in $(seq 1 ${tp});do
    ...
    path_qc=$(echo ${output_path}/${id}/qc/${id}_${visit_tp}_stx2_nlin.jpg)
    if [ ! -f ${path_qc} ];then 
