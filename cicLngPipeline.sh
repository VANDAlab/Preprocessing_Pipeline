...
for i in $(cat ${input_list});do
    ...
    bestlinreg_g ${t1} ${model_path}/Av_T1.mnc ${output_path}/${id}/${visit}/stx_lin/${id}_${visit}_t1_to_icbm_stx_tmp.xfm -clobber
...
if [ ${tp} -gt 1 ];then
    for timepoint in $(seq 1 ${tp});do
        ...
        if [ ${timepoint} = 1 ];then 
            bestlinreg_g ${output_path}/${id}/${visit_tp}/native/${id}_${visit_tp}_t1_vp.mnc ${model_path}/Av_T1.mnc ${output_path}/${id}/template/${id}_baseline_to_icbm_stx.xfm -clobber 
