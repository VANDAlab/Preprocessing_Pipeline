...
if [ ${tp} -gt 1 ];then
    for timepoint in $(seq 1 ${tp});do
        ...
        if [ ${timepoint} = 1 ];then 
            ...
            xfminvert ${output_path}/${id}/tmp0_GenericAffine.xfm ${output_path}/${id}/template/${id}_baseline_to_icbm_stx.xfm
...
for timepoint in $(seq 1 ${tp});do
    ...
    if [ ! -z ${flr} ];then echo ${id}_${visit_tp}_t1,${output_path}/${id}/${visit_tp}/stx_lin/${id}_${visit_tp}_t1_stx2_lin.mnc,\
...
if [ ! -z ${flr} ];then  python ${model_path}/BISON.py -c RF0 -m ${model_path}/Pretrained_Library_BISON_UKBB/ \
 -o  ${output_path}/${id}/tmp/ -t ${output_path}/${id}/tmp/ -e PT -n  ${output_path}/${id}/to_segment_t1_flair.csv  -p  ${model_path}/Pretrained_Library_BISON_UKBB/ -l 9; fi
